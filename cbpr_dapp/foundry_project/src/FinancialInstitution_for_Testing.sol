...
contract FinancialInstitution_for_Testing{

    address _ownerAddress;
    address _testAddress;

    mapping(address => string) public nostroAccountNo;
    mapping(string => mapping(address => bool)) public generalAccountExists;
    mapping(string => bool) public accountNoExists;
    ...
    function initiate_transfer(DbtrInstruction memory dbtrInstruction) public
    {
        // acting as debtor agent

        require(generalAccountExists[dbtrInstruction.DbtrAcct][msg.sender], "Sender (Debtor) is not an account holder of this institution!");
        require(balances[dbtrInstruction.DbtrAcct] >= dbtrInstruction.IntrBkSttlmAmt.Amt, "Sender (Debtor) doesn't have sufficient balance in their account!" );

        balances[dbtrInstruction.DbtrAcct] -= dbtrInstruction.IntrBkSttlmAmt.Amt;

        emit PassISOMessageAlong(dbtrInstruction.DbtrAgtIsoMsg, dbtrInstruction.NxtAgt);
    }
    ...
    function get_fresh_account_number(string memory wantedAcctNo) public pure returns (string memory)
    {
    ...
    function checkGeneralAccountExists(string memory accountNo, address user) public view returns (bool) {
        return generalAccountExists[accountNo][user];
    }
